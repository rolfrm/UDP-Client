...
u64_dlog * u64_dlog_create(const char * optional_name){
  ...
  static const char * const column_types[] = {"u64", "datalog_item_data"};
  u64_dlog * instance = calloc(sizeof(u64_dlog), 1);
  instance->column_names = (char **)column_names;
  instance->column_types = (char **)column_types;
  
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(u64), sizeof(datalog_item_data)}, (char *[]){(char *)"key", (char *)"value"});
...
bool u64_dlog_try_get(u64_dlog * table, u64 * key, datalog_item_data * value){
  ...
  unsigned int sizes[] = {sizeof(u64), sizeof(datalog_item_data)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
